...
After running the AIC stepwise  using both directions , we can see that several predictors were left out of the model due to their lack of  contribution . Among those features were 

* Employee status
* Income
* dt03 - Diet of low salt
* dt06 - high/low fiber diet

Are fiber and salt diets relted to Body mass? does income correlate with BMI directly or possibly with the interaction of some other feature? (we will see that interacted with Gender does bring a contribution)

Also notice in the report that per categorical feature , we represent N classes using N-1 features.

This leaves us with a smaller dimension of a problem , which can help us create a more generalized problem while staying away from the overfitting "curse".
...
We can add the interaction with grade to enhance the correlation between them , removing observations that might be older but with a low grade.

Age along with Low calorie diets are  correlated with BMI, with a T stat ~3.

Race interacted with Gender gives a very significant predictor with a Tstat of 4.3 and a pval 1.2xE-5. Specifically Race 2 (Black). What characterizes this race more that gives us a significant predictor vs other classes of this category? Specifically per a specific gender?
...
s_df_scaled <- s_df
s_df_scaled[c('age','income')] <- lapply(s_df_scaled[c('age','income')], function(x) c(scale(x)))
...
# take the ideal number of clusters and project and assign labels.
rang_of_clusters_to_plot<-2:15 #test clusters from 2 to 15
runs_per_cluster <-100 #Run the K Means algorithm 100 times per cluster
avg.totw.ss <-integer(length(rang_of_clusters_to_plot)) #initiate total withiness vector
for(v in rang_of_clusters_to_plot){ 
  v.totw.ss <-integer(runs_per_cluster) 
  for(i in 1:rang_of_clusters_to_plot){
    k.temp <-kmeans(s_df_scaled,centers=v) #Run kmeans per specific protocol
    v.totw.ss[i] <-k.temp$tot.withinss#save per run the total withiness 
  }
  avg.totw.ss[v-1] <-mean(v.totw.ss) #Average total withiness per all runs for current set of parameters and cluster
}

# plot out the avg withiness per cluster and find the optimal point using the elbow method.
plot(rang_of_clusters_to_plot,avg.totw.ss,type="b", main="Total Within SS by K",
...
Another note here that should be taken is the addition of all the rest of the features (survey questions). I have added these features in the assessment (the survey question features) and they have been found to be significant , especially using interactions between them and the demographic data.
...
Adding more of the features indeed give us a better description of the BMI and featerize our model better. The assuption is that by adding the information of the surveys (kq questions) we will get an even better model. from the initial results I saw that i recieve aournd an R square of 0.47 , so there is potential there.
